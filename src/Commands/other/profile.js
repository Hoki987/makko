        ...
        switch (true) {
            case isAssistant:
                if (target.user.bannerURL() === null) {
                    ...
                    let staffSheet = docAssist.sheetsById[0]
                    const sheet = staffSheet;
                    await sheet.loadCells()
                    const getRows = await sheet.getRows();
                    const rowTarget = getRows.find((r) => r._rawData.includes(interaction.user.id || interaction.options.getUser('пользователь').id))

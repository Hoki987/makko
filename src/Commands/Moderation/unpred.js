...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("unpred")
        .setDescription("Снимает предупреждение")
    ...
    async execute(client, interaction) {
        ...
        const text = {
            standart: `У пользователя <@${getUser.user.id}> было **снято** предупреждение`,
            badTwo: `\`\`\`Недостаточно прав!\`\`\``,
            badThree: `\`\`\`У пользователя нет предупреждения!\`\`\``,
        }
        const field = {
        ...
        switch (true) {
            ...
            case hasRole(WorkRoles.Pred) && await countStaff(interaction.user.id) === 0 && !hasRoleExecutor(StaffRoles.Admin) && ![OwnerId.hoki].includes(interaction.user.id):
            ...
            case !hasRole(WorkRoles.Pred):
                ...
                break;
            default:
                const lastPred = await findOneDB(getUser.user.id, 'Pred', { [Op.gt]: new Date() })
                switch (staffSheet) {
                    case 0:
                    case 1162940648:
                        switch (true) {
                            ...
                            case await countStaff(interaction.user.id) != 0:
                                ...
                                if (lastPred) {
                                    await History.update({ expiresAt: lastPred.createdAt.getTime() }, { where: { id: lastPred.id } })
        ...
        const embedDM = new EmbedBuilder().setDescription(`С вас сняли предупреждение!`).setColor(Utility.colorGreen).setFooter({ text: `Выполнил(а) ${interaction.user.tag} | ` + 'Сервер ' + interaction.guild.name, iconURL: interaction.user.displayAvatarURL() });

...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        const text = {
            standart: `**[${HistoryEmojis.BanJPG}] Пользователю <@${getUser.user.id}> был выдан <@&${WorkRoles.BanJPG}> на 14 дней\n\`\`\`ansi\n[2;35m[2;30m[2;35mПричина:[0m[2;30m[0m[2;35m[0m [2;36m${getReason}[0m\`\`\`**`,
            badOne: `**[${HistoryEmojis.BanJPG}] Пользователю <@${getUser.user.id}> не было выдано <@&${WorkRoles.BanJPG}>\n\`\`\`ansi\n[2;35m[2;30m[2;35mПричина:[0m[2;30m[0m[2;35m[0m [2;36mуже имеется banJPG[0m\`\`\`**`,
            badTwo: `\`\`\`Недостаточно прав!\`\`\``,
            badThree: `**[${HistoryEmojis.BanJPG}] Активные записи отсутствуют! BanJPG будет снят.**`,
            Appel: `\`\`\`ansi\n[2;35m[2;30m[2;35mПричина:[0m[2;30m[0m[2;35m[0m [2;36m${getReason}[0m\`\`\``
        }
        const field = {
        ...
        switch (true) {
            ...
            case hasRole(WorkRoles.BanJPG) && await countStaff(interaction.user.id) === 0 && !hasRoleExecutor(StaffRoles.Admin || StaffRoles.Developer || StaffRoles.Moderator) && ![OwnerId.hoki].includes(interaction.user.id):
                badDescription = text.badTwo;
                fields = field.Bad
                color = Utility.colorDiscord;
                break;
            case hasRole(WorkRoles.BanJPG):
                const activeBanJPG = await History.findOne({ where: { target: getUser.user.id, type: 'BanJPG', expiresAt: { [Op.gt]: new Date() }, } })
                if (activeBanJPG) {
                    ...
                    case null:
                        switch (true) {
                            ...
                            case await countStaff(interaction.user.id) != 0:
                                ...
                                color = Utility.colorDiscord
                                await createDB(interaction.user.id, getUser.user.id, getReason, 'BanJPG', new Date(Date.now() + 1209600000))

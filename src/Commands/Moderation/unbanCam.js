...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("unbancam")
        .setDescription("Снимает запрет камеры")
    ...
    async execute(client, interaction) {
        ...
        const text = {
            standart: `У пользователя <@${getUser.user.id}> был **снят** запрет камеры`,
            badTwo: `\`\`\`Недостаточно прав!\`\`\``,
            badThree: `\`\`\`У пользователя нет запрета камеры!\`\`\``,
        }
        const field = {
        ...
        switch (true) {
            ...
            case hasRole(WorkRoles.BanCam) && await countStaff(interaction.user.id) === 0 && !hasRoleExecutor(StaffRoles.Admin) && ![OwnerId.hoki].includes(interaction.user.id):
            ...
            case !hasRole(WorkRoles.BanCam):
                ...
                break;
            default:
                const lastBanCam = await findOneDB(getUser.user.id, 'BanCam', { [Op.gt]: new Date() })
                switch (staffSheet) {
                    case 0:
                    case 1162940648:
                        switch (true) {
                            ...
                            case await countStaff(interaction.user.id) != 0:
                                ...
                                if (lastBanCam) {
                                    await History.update({ expiresAt: lastBanCam.createdAt.getTime() }, { where: { id: lastBanCam.id } })
        ...
        const embedDM = new EmbedBuilder().setDescription(`С вас сняли запрет камеры!`).setColor(Utility.colorGreen).setFooter({ text: `Выполнил(а) ${interaction.user.tag} | ` + 'Сервер ' + interaction.guild.name, iconURL: interaction.user.displayAvatarURL() });
